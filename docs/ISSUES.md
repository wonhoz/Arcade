...
> **`sfz2aljk` 만 체크섬 경고가 남아 있다.** 받은 `sfz2al` 후보 4개를 전부 대조했지만
> 넷 다 `szajk.03a`(CRC `3c545a2a`)를 갖고 있지 않다 — 그 파일은 **부모가 아니라 한글패치 zip 소속**이라
> 부모셋을 바꿔서는 해결되지 않는다. 지금 `sfz2aljk.zip` 의 것은 `3b795a57` 로 다른 빌드다.
> **경고만 뜨고 구동은 정상**이라(실제 창으로 6초 만에 종료 확인) 그대로 두었다.
> 깔끔하게 맞추려면 0.224 가 기대하는 판의 `sfz2aljk.zip` 이 필요하다.
