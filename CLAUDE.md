...
> | **그 밖의 종료** | 에뮬레이터가 ESC 를 받든 안 받든 매번 같은 사다리 | ESC 로 두 번 연속 못 끝낸 에뮬레이터는 **기억해 두고 건너뛴다** — 기억은 `logs\esc-memory.json` 에 남아 **다음 실행에도 이어진다**. Demul 실측 11.9초 → **2.65초**, IDZ 21초 → **2.8초** |
>
> **ESC 기억(`logs\esc-memory.json`)** — 에뮬레이터 정의 이름마다 `Fails`(연속 실패) · `Exe`(실행파일 크기:수정시각) · `TriedAt`(마지막으로 ESC 를 실제로 보낸 시각).
> `Fails` 가 2 이상이면 `-Fast` 에서 사다리를 건너뛴다. **실행파일이 바뀌면 기억을 버리고, 7일이 지나면 한 번 다시 ESC 를 보내 본다.**
> MAME 계열은 대상이 아니다(강제 종료하면 `default.cfg` 가 잘리므로 항상 사다리를 탄다). `logs\` 라 장비마다 따로 쌓인다.
> 잊게 하려면 파일을 지운다(`docs/ISSUES.md` 97번).
>
...
`ShutdownMs`(종료 처리), `StrUsed`(`-str` 을 붙였는가), `Fingerprint`(입력의 지문 — `-Adaptive` 가 이걸로 갈라낸다), `Skipped`.
...
| `NOINPUT` | 그 에뮬레이터가 요구하는 **입력장치(마우스·키보드)가 연결돼 있지 않아 띄우지 않았다**(경고). Demul 은 마우스가 없으면 60여 건이 통째로 실패하고(76번), PSXMAME 도 같은 상태에서 코드 100 으로 끝났다. 꽂고 다시 돌리면 그 항목만 다시 검사한다(`docs/ISSUES.md` 99번) |
| — | `-Adaptive` 로 건너뛴 항목은 **직전 상태를 그대로 물려받고** `Skipped` 열이 `True` 가 된다. 화면에는 `PASS~` 처럼 `~` 가 붙는다 |
...
- **전수 점검은 몇 시간짜리다.** 그래서 `-Launch` 는 **5건마다 보고서를 써 두고**, 중단하면 `-Resume` 이
  ...
  - **5건마다 쓰는 것은 CSV 뿐이다.** HTML(1,098행 기준 한 번 약 0.2초)은 **실제로 띄운 항목이 5건 쌓였거나,
    띄운 항목이 있는 채 60초가 지났을 때**와 끝날 때만 쓴다. 거의 다 건너뛰는 증분 점검에서 HTML 을 220번
    다시 쓰던 낭비다 — IDZ 한 건만 띄우는 증분 점검이 48초 → 37초(IDZ 23초를 빼면 25초 → 14초)가 됐다(`docs/ISSUES.md` 96번).
    CSV 를 줄이지 않은 이유: `[4] 이어하기` 는 `-Adaptive` 없이 `-Resume` 만 쓰므로, 저장되지 않은 "건너뛴 항목"은 이어하기에서 **실제로 띄워진다.**
